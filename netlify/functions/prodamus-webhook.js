...
// Логика обработки веб-хуков Prodamus
...
exports.handler = async (event) => {
    ...
    try {
        ...
        // Если в запросе есть 'payment_status', это веб-хук от Prodamus
        if (requestBody.payment_status) {
            return await handleProdamusWebhook(client, requestBody);
        }

        // В противном случае, это запрос на авторизацию
        ...
        if (res.rows.length > 0) {
            ...
            return {
                ...
                headers: {
                    ...
                    'Set-Cookie': `token=${token}; Path=/; Domain=.proculinaria-book.ru; HttpOnly; Secure; SameSite=Lax; Max-Age=86400`,
                },
            };
        } else {
            return {
        ...
        console.error('Ошибка в serverless-функции:', err.message);
        return {
            statusCode: 500,
            body: JSON.stringify({
